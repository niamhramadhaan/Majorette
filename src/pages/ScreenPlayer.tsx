...
export default function ScreenPlayer() {
  ...
  const activeSchedule = sid ? getActiveScheduleForScreen(schedules, content, sid) : null;
  const upcomingSchedule = !activeSchedule && sid ? getUpcomingScheduleForScreen(schedules, content, sid) : null;
  ...
  useEffect(() => {
    ...
    const skipEvent = `skip-signal-${sid}`;
    const pauseEvent = `pause-signal-${sid}`;
    const resumeEvent = `resume-signal-${sid}`;
    const doneEvent = `done-signal-${sid}`;

    window.addEventListener(skipEvent, onSkip);
    const onStorage = (e: StorageEvent) => {
      if (e.key === `jemima_skip_signal_${sid}` && e.newValue) {
        try { const signal = JSON.parse(e.newValue); if (signal?.direction) handleSkipSignal(signal.direction); } catch {}
      }
      if (e.key === `jemima_pause_signal_${sid}`) setIsPlaying(false);
      if (e.key === `jemima_resume_signal_${sid}`) setIsPlaying(true);
      if (e.key === `jemima_done_signal_${sid}`) { setIsPlaying(false); setScheduleDone(true); }
    };
    window.addEventListener('storage', onStorage);
    const onPause = () => setIsPlaying(false);
    window.addEventListener(pauseEvent, onPause);
    const onResume = () => setIsPlaying(true);
    window.addEventListener(resumeEvent, onResume);
    const onDone = () => { setIsPlaying(false); setScheduleDone(true); };
    window.addEventListener(doneEvent, onDone);
    return () => {
      window.removeEventListener(skipEvent, onSkip);
      window.removeEventListener('storage', onStorage);
      window.removeEventListener(pauseEvent, onPause);
  ...
  if (scheduleItems.length > 0 && isOverlay(scheduleItems[visualIndex], content, visualIndex, scheduleItems)) {
    ...
    for (let i = visualIndex + 1; i < scheduleItems.length; i++) {
      if (!isOverlay(scheduleItems[i], content, i, scheduleItems)) { visualIndex = i; visualOffset = 0; found = true; break; }
    }
    if (!found) {
      for (let i = 0; i < scheduleItems.length; i++) {
        if (!isOverlay(scheduleItems[i], content, i, scheduleItems)) { visualIndex = i; visualOffset = 0; break; }
      }
    }
  }
  ...
  if (!overlayAtPosition) {
    for (let i = rawResult.index + 1; i < scheduleItems.length; i++) {
      if (isOverlay(scheduleItems[i], content, i, scheduleItems)) {
        const si = scheduleItems[i]; const ci = content.find(c => c.id === si.contentId);
  ...
  const visualItem = scheduleItems[visualIndex] ? content.find(c => c.id === scheduleItems[visualIndex].contentId) : null;
  const visualItemDuration = visualItem ? (scheduleItems[visualIndex]?.duration || visualItem.duration) : 0;

  useEffect(() => { muteInitializedRef.current = false; }, [activeSchedule?.id]);

  useEffect(() => {
    if (!visualItem || !activeSchedule || muteInitializedRef.current) return;
    muteInitializedRef.current = true;
    const screenMute = screen?.muteConfigOverride;
    const config = screenMute || getSettings().playerMuteConfig;
    if (visualItem.type === 'image') setIsMuted(config.image);
    else if (overlayAtPosition) setIsMuted(visualItem.type === 'video' ? config.videoWithOverlay : config.audioNoOverlay);
    else setIsMuted(visualItem.type === 'video' ? config.videoNoOverlay : config.audioNoOverlay);
  ...
  useEffect(() => {
    if (bgAudioLabel && isMuted && !controlsLockedRef.current) {
      setShowControls(true);
      if (controlsTimeoutRef.current) clearTimeout(controlsTimeoutRef.current);
      controlsTimeoutRef.current = window.setTimeout(() => { if (isPlaying && !mediaError) setShowControls(false); }, 5000);
    }
  }, [bgAudioLabel]);
  ...
  const handleMediaTimeUpdate = () => {
    ...
    if (visualItem.type === 'video' && mainVideoRef.current) setProgress(Math.min((mainVideoRef.current.currentTime / visualItemDuration) * 100, 100));
    else if (visualItem.type === 'audio' && mainAudioRef.current) setProgress(Math.min((mainAudioRef.current.currentTime / visualItemDuration) * 100, 100));
  };
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (visualItem.type === 'video' && mainVideoRef.current) mainVideoRef.current.currentTime = seekTime;
    else if (visualItem.type === 'audio' && mainAudioRef.current) mainAudioRef.current.currentTime = seekTime;
    setProgress(pct * 100);
  };
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-2">No screen ID provided</p>
          <p className="text-gray-600 text-sm">Use the screen player URL from Locations.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          {screen && <p className="text-gray-500 text-sm mt-1"><Monitor className="w-3 h-3 inline mr-1" />{screen.name}</p>}
    ...
    if (upcomingSchedule) {
      ...
      return (
        <div className="min-h-screen bg-black relative overflow-hidden">
          {thumbnail && (<><img src={thumbnail} alt="" className="absolute inset-0 w-full h-full object-cover opacity-30" /><div className="absolute inset-0 bg-black/60" /></>)}
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center z-10">
              ...
              {screen && <p className="text-gray-500 text-sm mt-2"><Monitor className="w-3 h-3 inline mr-1" />{screen.name}</p>}
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div ref={containerRef} className={cn("min-h-screen bg-black relative overflow-hidden cursor-none")} onMouseMove={handleMouseMove} onMouseLeave={() => !controlsLockedRef.current && isPlaying && !mediaError && setShowControls(false)}>
      <div className="absolute inset-0 flex items-center justify-center">
        {visualItem.type === 'video' && (
          <video ref={mainVideoRef} className="w-full h-full object-contain" muted={isMuted} playsInline
            onTimeUpdate={handleMediaTimeUpdate} onError={() => handleMediaError(visualItem.fileName)}
        ...
        <audio ref={bgAudioRef} playsInline onEnded={() => { bgAudioIdRef.current = null; setBgAudioLabel(null); }} />

        {bgAudioLabel && (<div className="absolute top-4 right-14 opacity-30"><Music className="w-6 h-6 text-white" /></div>)}
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {screen && <span className="text-[10px] text-gray-500"><Monitor className="w-2.5 h-2.5 inline mr-0.5" />{screen.name}</span>}
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setIsMuted(!isMuted)} className={cn("p-2 text-white hover:text-secondary transition-colors relative", bgAudioLabel && isMuted && "animate-pulse text-secondary")}>
                ...
                {bgAudioLabel && isMuted && (
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-black/80 text-white text-[10px] px-2 py-1 rounded animate-pulse">Click to unmute</span>
                )}
              </button>
      ...
      {showControls && activeSchedule && (
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          {screen && <span className="px-3 py-1 bg-black/50 rounded-full text-xs text-white font-medium flex items-center gap-1"><Monitor className="w-3 h-3" />{screen.name}</span>}
        </div>
      )}

      <div className="absolute bottom-4 right-4 flex items-center gap-2">
        <button onClick={() => setShowShortcuts(prev => !prev)}
          className={cn("p-2 rounded-full transition-all", showShortcuts ? "bg-white/20 text-white" : "text-gray-500 hover:text-white hover:bg-white/10 opacity-40 hover:opacity-100")}
          title="Keyboard shortcuts (?)">
          <Keyboard className="w-4 h-4" />
        </button>
        <button onClick={handleLockToggle}
          className={cn("p-2 rounded-full transition-all", controlsLocked ? "text-white/30 hover:text-white/60" : "text-gray-500 hover:text-white hover:bg-white/10 opacity-40 hover:opacity-100")}
      ...
      {showShortcuts && (
        <div className="absolute bottom-16 right-4 bg-black/90 backdrop-blur-sm rounded-xl border border-white/10 p-4 min-w-[200px] animate-in fade-in slide-in-from-bottom-2">
          ...
          <div className="space-y-2">
            {[{ key: 'Space', desc: 'Play / Pause' }, { key: 'F', desc: 'Toggle fullscreen' }, { key: 'M', desc: 'Mute / Unmute' }, { key: 'L', desc: 'Lock / Unlock controls' }, { key: '?', desc: 'Toggle this help' }].map(({ key, desc }) => (
